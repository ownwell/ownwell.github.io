    ...
    void apply(Project target) {
        ...
        if (!isForceInsert) {
            ...
            for (int index = 0; index < taskNames.size(); ++index) {
                ...
                //FIXME: assembleRelease下屏蔽Prepare，
                // 这里因为还没有执行Task，没法直接通过当前的BuildType来判断，所以直接分析当前的startParameter中的taskname，
...
//                    logger.warn " Don't register robust transform for debug model !!! task is：${taskName}"
    ...
    def initConfig() {
        ...
        // 获取需要Robust插入代码的包名（有些类，我们不希望进行插桩）
        ...
        // fixme   这玩意是干嘛的
        if (null != robust.switch.filterMethod
                && "true".equals(String.valueOf(robust.switch.turnOnExceptMethod.text()))) {
            isExceptMethodLevel = true;
        }

        if (robust.switch.forceInsert != null
                && "true".equals(String.valueOf(robust.switch.forceInsert.text()))) {
            logger.quiet String.valueOf(String.valueOf(robust.switch.forceInsert.text()))
            isForceInsert = true
        }
    ...
    void transform(Context context, Collection<TransformInput> inputs, Collection<TransformInput> referencedInputs, TransformOutputProvider outputProvider, boolean isIncremental) throws IOException, TransformException, InterruptedException {
        ...
        logger.debug  "================ jarFile path = $jarFile"
...
//                            println(" class name "+ctClass.name+" should not be insert code ")
                        ...
                        // 是否添加changeQuickRedirect这个变量到这个类中
                        // 相当于在每个类中加入一个
...
//                            logger.debug "ctClass: " + ctClass.getName();
                        ...
                        if ((it.getModifiers() & AccessFlag.SYNTHETIC) != 0
                                && !AccessFlag.isPrivate(it.getModifiers())) {
                            return false
                        }
